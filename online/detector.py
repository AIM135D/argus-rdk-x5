...
def ascii_zone_label(zone_item, stat=None):
    """Build OpenCV-safe ASCII zone label.

    cv2.putText does not render Chinese glyphs with Hershey fonts; on RDK/OpenCV it
    often becomes question marks. Keep video overlays ASCII-only and leave Chinese
    names to the web UI/logs.
    """
    stat = stat or {}
    try:
    ...
    if bool(zone_item.get("require_vest", False)):
        rules.append("V")
    ppe = "+".join(rules) if rules else "-"
    if not bool(zone_item.get("enabled", True)):
        return f"[OFF] Zone-{zone_id}"
    risk_score = 0.0
    try:
        risk_score = float(stat.get("max_risk_score", 0.0) or 0.0)
    except Exception:
        risk_score = 0.0
    risk_text = f" R:{risk_score:.2f}" if bool(stat.get("active", False)) else ""
    return f"[{risk}] Zone-{zone_id} PPE:{ppe}{risk_text}"
...
def draw_results(frame: np.ndarray, detections):
    ...
    for zone_item in danger_zones:
        ...
        # OpenCV Hershey fonts cannot render Chinese reliably.
        # Keep video overlay ASCII-only to avoid labels becoming "????".
        tag = ascii_zone_label(zone_item, stat)
        cv2.putText(
            draw_frame,
            tag,
